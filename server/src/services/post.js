...
export const updatePostByAdmin = (
  { title, area, price, isActived, address, aid },
  pid
) =>
  new Promise(async (resolve, reject) => {
    try {
        ...
        db.Post.update(
          {
            title,
            address,
            priceNumber: Math.round(price / 100000) / 10,
            areaNumber: area,
            isActived,
          },
          {
        ...
        db.Attribute.update(
          {
            price:
              price < 1000000
                ...
                : `${Number(
                    (Math.round(price / 100000) / 10).toFixed(1)
                  ).toLocaleString()} triệu/tháng`,
            acreage: `${area}m2`,
          },
          {
            where: { id: aid },
